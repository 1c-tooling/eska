...
корневого descriptor реализованы в [T61](metadata-sources.md); сама проверка типа не требует платформы
1С или заполненного `[build].platform_version`.

## Матрица типов

Единый справочник содержит 67 логических типов, включая WebSocketClient
(web-socket-client, каталог WebSocketClients). Алиасы внешних descriptors
...
| Command | `command` | — |
| Attribute | `attribute` | — |
| AddressingAttribute | `addressing-attribute` | реквизит адресации задачи |
...
В T65 добавлен `AddressingAttribute`, найденный в XML задачи большого стенда.
`CollectionKind::Unsupported` сохраняет видимую ветвь неподдержанных фрагментов.
[Схемы и дерево](configurator-tree.md) используют эти идентификаторы без изменения
формата прежних IDs и существующего CLI JSON.
