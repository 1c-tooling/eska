...
`NEXT` — T78. Декомпозиция backend, поиска и открытых решений —
...
подготовка `eska` и расширения VS Code для Designer XML. Текущий `NEXT` — T78.
...
15. T78 — поиск и переход к результату, NEXT
