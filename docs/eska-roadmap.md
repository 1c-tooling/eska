...
`NEXT` — T60. Декомпозиция backend, поиска и открытых решений —
...
версии сборки для этих проверок не требуются. Охват поиска/фильтра и остальные
неподтверждённые UX-предложения перечислены в плане T76–T81.
...
подготовка `eska` и расширения VS Code для Designer XML. Текущий `NEXT` — T60.
...
```text
1. T60 — модель метаданных и logical identity, NEXT
2. T61 — обязательный manifest, Designer resolver и source mapping
