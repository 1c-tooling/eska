...
T35 завершена: `switch` сохраняет и восстанавливает полки веток; доступны
явные `shelve`, `unshelve`, `shelves`, JSON и read-only preview.

Ближайшие задачи выполнять в таком порядке, если не принято новое решение:

```text
1. T56 — version bump auto и подготовка Release PR/MR, NEXT
2. T57 — update установленного eska до последнего стабильного release
3. T37 — sync / continue / abort
4. T55 — завершение внешних Windows/macOS и реальных 1С-проверок при доступном runner
5. T26 — fmt, последняя задача текущей очереди
```

T55 можно завершить раньше T37, если станет доступна требуемая ОС; ожидание
внешнего runner не блокирует T56, T57 и T37 на текущей системе.
...
T54 и T58–T59 завершены; основа T55 реализована,
