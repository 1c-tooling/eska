...
T75 добавляет поиск нераскрытых объектов с отдельной индексацией; начало работы
...
`NEXT` — T62. Декомпозиция backend, поиска и открытых решений —
...
подготовка `eska` и расширения VS Code для Designer XML. Текущий `NEXT` — T62.
...
3. T62 — fixtures, матрица типов и большой стенд, NEXT
4. T63 — выбор XML parsing и исходные замеры
5. T64 — разбор Designer XML по запросу
6. T65 — схемы дерева, группа модулей и пустые коллекции
7. T66 — read-only metadata API
8. T67 — lazy cache и инкрементальное обновление
9. T75 — индекс и API поиска
10. T68 — производительность backend
11. T69 — спецификация IDE protocol
12. T70 — постоянный процесс eska ide --stdio
