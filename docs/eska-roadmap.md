...
Принят [IDE protocol 1.0](ide-protocol.md); команда реализуется в T70.
`NEXT` — T70. Декомпозиция backend, поиска и открытых решений —
...
подготовка `eska` и расширения VS Code для Designer XML. Текущий `NEXT` — T70.
...
12. T70 — постоянный процесс eska ide --stdio, NEXT
