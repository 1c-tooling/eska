...
Статус: спецификация принята 2026-09-18; **реализация команды — T70**.
Команда `eska ide --stdio` пока не добавляется в пользовательский CLI.
...
это примеры контракта, **не результаты запуска ещё не реализованного сервера**.
Их значения IDs/путей иллюстративны, воспроизводимые данные core находятся
в tests/fixtures/designer. Для reveal T70 добавляет точечный core lookup записи индекса по ObjectId;
