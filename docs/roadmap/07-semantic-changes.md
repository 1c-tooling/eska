...
Оптимизация чтения снимков: file-level и semantic workspace-анализ используют
общий HEAD/index reader в пределах каждого анализа вместо повторного разрешения
HEAD и проверки индекса для каждого файла. Reader не хранится между операциями;
CLI, JSON-схемы и алгоритмы определения событий сохранены. BSL parser вынесен в
`project/semantic/routines.rs` и читает строки без промежуточных массивов;
XML-сигнатуры дописываются в общий буфер без копирования поддеревьев.

Исправление по ревью 2026-09-24 — **DONE**: сравнение XML учитывает namespace URI
элементов и атрибутов, отличает текст с экранированной разметкой от дочерних
элементов и игнорирует комментарии/processing instructions. Префиксы namespace
и форматирование не влияют на результат; JSON-схемы и виды событий сохранены.
