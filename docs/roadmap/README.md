...
- `NEXT`: `T58` — настройка основной production-ветки Git Flow без изменения
  базы feature-веток;
- `PLANNED`: `T59` — читаемый semantic-вывод `eska diff` без дублей и с порядком
  метаданных Конфигуратора после T58;
...
| 2 | T59 | Читаемый semantic-вывод `eska diff` без дублей и с порядком Конфигуратора |
...
| T59 | PLANNED | Читаемый и упорядоченный semantic-вывод `eska diff` | [04-core-vcs-ux.md](04-core-vcs-ux.md) |
