...
- `DONE`: `T58` — настройка основной production-ветки Git Flow без изменения
  базы feature-веток;
- `NEXT`: `T59` — читаемый semantic-вывод `eska diff` с координатами методов,
...
сохранением `develop` базой обычных feature-веток. Перед расширением переключения
T35 следующей выполняется T59: human-вывод `diff` должен однозначно называть
источник изменений.
...
|---|---|---|
| 1 | T59 | Читаемый semantic-вывод `eska diff` с координатами методов и без дублей |
| 2 | T35 | Сохранение состояния задачи и автоматическое восстановление при `switch` |
| 3 | T56 | Автоматический bump версии 1С и подготовка Release PR/MR |
| 4 | T57 | Обновление установленного `eska` до последнего стабильного release |
...
внешнего runner не блокирует реализацию T59, T35, T56, T57 и T37 на
текущей системе.
...
| T59 | NEXT | Читаемый semantic-вывод `eska diff` с координатами методов | [04-core-vcs-ux.md](04-core-vcs-ux.md) |
