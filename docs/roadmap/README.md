...
- `PLANNED`: `T35` — автоматическое сохранение и восстановление незакоммиченного
  состояния при переключении задач после T58;
...
сначала выполнить `save`. До расширения переключения T35 следующей выбрана T58:
Git Flow должен позволять назвать production-ветку `master`, сохраняя `develop`
базой и целью интеграции обычных feature-веток.
...
внешнего runner не блокирует реализацию T58, T35, T56, T57 и T37 на текущей
системе.
