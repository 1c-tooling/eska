...
Задача отложена до проверки практического CLI MVP (`switch`, `finish`, `build`)
в реальной разработке. Это меняет приоритет, но не удаляет задачу и её контракт.
...
В текущей очереди задача выполняется последней.
