...
- после имени метода выводится его сигнатура с параметрами; параметры сохраняют
  порядок и имена объявления, а
  многострочная декларация нормализуется в одну строку human-вывода;
- для добавленного метода показывается 1-based номер строки его объявления в
  новом снимке, например:

  ```text
  ОбщийМодуль.ОбменСМастерСистемой — Процедура.ДобавитьОбработаннуюСтроку (Строка, Колонка), строка 42
  ```

  В EN используется окончание `line 42`;
- conservative BSL parser сохраняет тип объявления, исходные имена параметров и
  строку начала, учитывает CRLF/LF и продолжает переходить к существующему
  fallback при неоднозначной или незавершённой декларации;
- маркеры, цвета TTY и отсутствие ANSI при redirect/`NO_COLOR` сохраняются;
- raw и схемы JSON не меняются: новые сигнатура и номер строки относятся только
  к human-проекции; semantic JSON содержит исправленный набор событий без пары
...
- integration-тесты фиксируют точные RU/EN human-заголовки и смысл счётчика для
  workspace и сравнения Git-ревизий, сигнатуру и строку метода, отсутствие
