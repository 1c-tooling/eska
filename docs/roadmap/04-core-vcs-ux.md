...
## T59 — Читаемый и упорядоченный semantic-вывод `eska diff`

**Статус:** `DONE`
**Зависит от:** T14, T21

Исправить подписи источника семантических изменений, чтобы заголовок вида
`Добавлен объект — ревизии (2)` не воспринимался как число ревизий объекта 1С.
Human-вывод должен явно показывать, что событие найдено между двумя Git-
ревизиями, в index или в рабочей копии, а число в скобках относится к количеству
событий в подгруппе. Одновременно устранить противоречивые события для одного
логического объекта и упорядочить разделы метаданных как в дереве Конфигуратора.

**Готово, когда:**

- RU/EN подписи читаются как отношения или состояния, а не как названия
  сущностей: например, `между ревизиями` вместо отдельного слова `ревизии`;
- заголовки остаются компактными и однозначными для revision, index, worktree и
  совмещённых index/worktree изменений;
- термин `Git-ревизия` не смешивается с версией проекта 1С и изменениями
  отдельного объекта метаданных;
- один логический объект не выводится одновременно добавленным и изменённым:
  lifecycle-событие добавления или удаления подавляет производные события
  изменения формы, модуля и свойств с той же identity в рамках одного сравнения;
- подавление учитывает точную identity и границы владения: добавление формы не
  скрывает независимые изменения документа или других его форм;
- добавленная `Документ.<Имя>.Форма.<Имя>` из воспроизводящего сценария остаётся
  только в группе добавленных объектов и не повторяется как изменённые форма и
  модуль;
- разделы типов метаданных выводятся в порядке дерева Конфигуратора: сначала
  «Общие», затем «Константы» и остальные разделы, а не в алфавитном порядке;
  порядок задаётся одной явной таблицей и имеет детерминированный fallback для
  неизвестного типа;
- фактический порядок сверяется с деревом метаданных поддерживаемой версии
  Конфигуратора, а не выводится из русских локализованных названий;
- группы процедур и функций объединяются по lifecycle в `Добавлен метод`,
  `Изменён метод` и `Удалён метод`; конкретный вид объявления переносится в
  строку события как `Процедура.<Имя>` / `Функция.<Имя>` в RU и
  `Procedure.<Name>` / `Function.<Name>` в EN;
- после имени метода в скобках выводятся 1-based координаты начала объявления
  `(<строка>, <колонка>)`, а не параметры метода; колонка указывает позицию
  ключевого слова `Процедура`/`Функция` в исходной строке;
- добавленный и изменённый метод используют координаты нового снимка, удалённый
  метод — старого, например:

  ```text
  ОбщийМодуль.ОбменСМастерСистемой — Процедура.ДобавитьОбработаннуюСтроку (42, 3)
  ```

- conservative BSL parser сохраняет тип объявления, строку и колонку начала,
  одинаково считает координаты для CRLF/LF и продолжает переходить к
  существующему fallback при неоднозначной или незавершённой декларации;
- маркеры, цвета TTY и отсутствие ANSI при redirect/`NO_COLOR` сохраняются;
- raw и схемы JSON не меняются: новые координаты относятся только к human-
  проекции; semantic JSON содержит исправленный набор событий без пары
  `added`/`changed` для одной identity, сохраняя текущие machine event kinds
  процедур и функций;
- integration-тесты фиксируют точные RU/EN human-заголовки и смысл счётчика для
  workspace и сравнения Git-ревизий, координаты метода, отсутствие
  дублей и порядок как в Конфигураторе.

Задача не меняет определение владельцев метаданных или схему сравнения Git.
Будущая [T65](13-metadata-and-ide.md) переиспользует таблицу порядка типов T59
в `ConfiguratorSchema`. T59 не ждёт реализации IDE-дерева и не включает его
дочерние коллекции или protocol.

**Реализовано:** human-вывод использует контекстные подписи `между
Git-ревизиями`, `в индексе`, `в рабочей копии` и совмещённое workspace-состояние.
Процедуры и функции объединены по lifecycle в группы методов; строка события
показывает локализованный вид объявления и 1-based координаты ключевого слова.
Внутри объекта методы отсортированы по этим координатам и повторяют порядок
объявлений в исходном файле.

Порядок разделов задаёт единая таблица дерева Конфигуратора с детерминированным
fallback для неизвестных типов. Lifecycle объекта подавляет производные события
формы, модуля, свойств и методов только для той же identity и comparison stage.
Raw и semantic JSON v4 сохраняют прежние поля и machine event kinds.

**Проверено:** `cargo fmt --check`, `cargo check`,
`cargo clippy --all-targets --all-features -- -D warnings`, 147 unit-тестов и
253 integration-теста — успешно; один ручной performance-тест штатно ignored.
На исходном сценарии `0945306b401381d1c44b70c8563a3d0ec5906fc6` проверены
RU/EN, порядок разделов, координаты добавленных и изменённых методов, отсутствие
дублей добавленных форм, сохранность JSON v4 и отсутствие ANSI при redirect и
`NO_COLOR`.
