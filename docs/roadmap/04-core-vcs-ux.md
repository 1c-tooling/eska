...
## T59 — Читаемые подгруппы human-вывода `eska diff`
...
- порядок групп, маркеры, цвета TTY и отсутствие ANSI при redirect/`NO_COLOR`
  сохраняются;
- JSON и raw output не меняются;
- integration-тесты фиксируют точные RU/EN human-заголовки и смысл счётчика для
  workspace и сравнения Git-ревизий.

Задача не меняет semantic-анализ, определение объектов или схему сравнения Git.
