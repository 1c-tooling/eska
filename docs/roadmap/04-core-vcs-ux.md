...
- raw и схемы JSON не меняются; semantic JSON содержит тот же исправленный набор
  событий без пары `added`/`changed` для одной identity;
- integration-тесты фиксируют точные RU/EN human-заголовки и смысл счётчика для
  workspace и сравнения Git-ревизий, отсутствие дублей и порядок как в
  Конфигураторе.
