...
status-project = Проект { $value }
status-project-type = Тип { $value }
status-workflow = Workflow { $value }
status-task = Задача { $value }
status-branch = Ветка { $value }
status-base = База { $value }
status-changes = Изменения
status-files = Файлов: { $count }
status-added = Добавлено: { $count }
status-modified = Изменено: { $count }
status-deleted = Удалено: { $count }
status-type-changed = Изменён тип: { $count }
status-untracked = Не отслеживается: { $count }
status-intent-to-add = Подготовлено к добавлению: { $count }
status-conflicts = Конфликтов: { $count }
status-synchronization = Синхронизация с удалённой базой
status-ahead = Локально впереди: { $count }
status-behind = Удалённо впереди: { $count }
