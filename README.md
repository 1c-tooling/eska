...
Команда сохраняет способ установки: Cargo обновляется через Cargo, установка
...
[Способы установки, параметры и JSON-контракт обновления](docs/cli-update.md).
...
Preview показывает выбранный project/workspace scope, все включаемые пути,
...
`eska` загружает этот `.cf` в управляемую служебную информационную базу и
...
В workspace доступны те же selectors: `eska clean -p <project>` и `eska clean
--workspace`.

Снимки исходников и незавершённые артефакты по-прежнему удаляются после
завершения, ошибки или прерывания.
...
Команда копирует Designer XML в закрытый временный снимок, собирает только из
...
Матрица переносимых тестов и отдельный протокол приёмки с настоящей платформой
1С описаны в [проверке сборки на поддерживаемых ОС](docs/build-validation.md).
...
Перед началом работы можно проверить проект, инструменты 1С и Git одним
read-only запуском:
...
`doctor` проверяет конфигурацию и корневой Designer XML-дескриптор, требуемую
...
Независимые проверки продолжаются после ошибки, зависимые получают статус
...
| `ide --stdio` | Запустить постоянный read-only backend дерева и поиска для IDE |
...
[IDE protocol 1.3](docs/ide-protocol.md), не CLI JSON и не LSP. Платформа 1С для
