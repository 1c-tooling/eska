...
`eska` создаёт временную информационную базу с этим `.cf` и передаёт `ibcmd
infobase config import` полный каталог XML-выгрузки Конфигуратора. Основная
информационная база не изменяется. Параметр доступен только для проектов
